...
export function Flashcard({ card, onDifficulty, onKnown, onUnknown, isDatabaseMode = true, onDrawToggle, isDrawingOpen = false, isCompactMode = false, showHanziHint = false, onHanziHintToggle }: FlashcardProps) {
  ...
  return (
    <div
      className={`w-full mx-auto perspective-1000 cursor-pointer flex flex-col items-center justify-center ${isCompactMode
        ...
        >
            ...
            <div className="absolute top-3 right-3 z-10">
              {onDrawToggle ? (
                <button
                  ...
                  className={`px-3 py-2 rounded-full text-xs font-semibold border backdrop-blur-md shadow-sm transition-all duration-300 hover:scale-105 active:scale-95 ${isDrawingOpen
                ...
                <div className={`px-2 py-1 rounded-full text-xs font-semibold border backdrop-blur-md shadow-sm ${(card as any).status === 'new' ? 'bg-blue-100 text-blue-800 border-blue-200' :
          ...
          <div className={`flex flex-col items-center justify-center h-full w-full ${isCompactMode ? 'space-y-1' : 'space-y-4'}`}>
            ...
            {isCompactMode && onHanziHintToggle ? (
              <div
                className={`font-bold text-blue-600 dark:text-blue-400 text-center ${isCompactMode ? 'text-2xl' : 'text-2xl sm:text-3xl mb-2'
                  }`}
              >
                {(() => {
                  if (showHanziHint) {
              ...
              <div className={`font-bold text-blue-600 dark:text-blue-400 text-center ${isCompactMode ? 'text-lg' : 'text-2xl sm:text-3xl mb-2'
                }`}>
                {(() => {
                  const isLevel2Card = card.id && card.id.startsWith('l2-');
            ...
            {/* English Meaning */}
            <div className={`text-gray-700 dark:text-slate-200 font-medium text-center ${isCompactMode ? 'text-sm' : 'text-lg sm:text-xl'
              }`}>
              {card.english}
            </div>
