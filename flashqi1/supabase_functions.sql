-- =================================================================
-- FLASHCARD DATABASE FUNCTIONS FOR SUPABASE
...
BEGIN
  ...
  LEFT JOIN user_flashcard_progress ufp ON f.id = ufp.flashcard_id AND ufp.user_id = p_user_id
  WHERE 
    -- Filter by difficulty and include reviewable cards
    (
      -- For 'all': show new cards + cards due for review + learning cards due
      (p_difficulty = 'all' AND (
        ufp.status IS NULL OR 
        ufp.status = 'new' OR 
        (ufp.status = 'learning' AND (ufp.next_review IS NULL OR ufp.next_review <= NOW())) OR
        (ufp.status = 'due' AND (ufp.next_review IS NULL OR ufp.next_review <= NOW()))
      ))
      OR
      -- For specific difficulty: show cards marked with that difficulty + new cards + cards due for review
      (p_difficulty != 'all' AND (
        ufp.last_difficulty = p_difficulty OR
        (ufp.last_difficulty IS NULL AND ufp.status IS NULL) OR
        (ufp.status = 'learning' AND ufp.last_difficulty = p_difficulty AND (ufp.next_review IS NULL OR ufp.next_review <= NOW()))
      ))
    )
  ORDER BY 
